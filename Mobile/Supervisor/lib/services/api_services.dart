import 'dart:convert';
import 'package:autine1/screens/welcome_screen.dart';
...
Future<dynamic> primRegister(BuildContext context,String email,String pass) async {
  ...
  var response = await http.post(
    Uri.parse('$baseUrl/auths/supervisor-register'),
  ...
  "country": "Egypt",
  "city": "Cairo",
  "dateOfBirth": "2020-09-29T00:00:00Z",
  ...
  if (response.statusCode == 200) {
    ...
    if (userId == null || vCode == null) {
      ...
      return response;
         ...
         await saveData(generateUname(email), "username");
...
/////////////////////////////////// Update user data ///////////////////////////////////////////////////////
 ...
 if (response.statusCode == 204) {
  ...
  Future<void> generateUsername(BuildContext context,String email) async {
  
    var response = await http.get(
      Uri.parse('http://grad-project-ai-api.vercel.app/auth/user/id?email=$email'),
        headers: {
    'Content-Type': 'application/json',
    },
 ...
 if (response.statusCode == 200) {
       final data = await jsonDecode(response.body);
       String? uname = data["user_id"];
       await saveData(uname.toString(), "username");
       print("Username is $uname");

    } else {
      print("Error generating username (Status code: ${response.statusCode})");
          ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text('Error generating username!')),
  );
...
Future<void> getData(BuildContext context,String token) async {
 ...
 if (response.statusCode == 200) {
       ...
       await saveData(id.toString(), "userId");
...
Future<bool> getToken(BuildContext context,String email , String pass) async {
  ...
  Future<bool> login(BuildContext context,String email , String password) async {
    ...
    if (response.statusCode == 200) {
      print("Login success");
      //await generateUsername(context,email);
      final data = await jsonDecode(response.body);
      String? token = data["accessToken"];
      String? reftoken = data["refreshToken"];
      await saveData(token.toString(), "token");
      await saveData(reftoken.toString(), "refToken");
      if (token == null) {
        print("Error: Missing token in response");
          }
